  ...
  >
    <div class="flex justify-between items-start">
      <div class="flex-1 flex flex-col gap-2">
        <div class="flex items-center gap-2">
          <h3 class="text-xl font-semibold text-[#4a90e2]">{{ sensor?.name || 'Sensor sin nombre' }}</h3>
        </div>
    ...
    <div class="flex flex-col gap-3 mt-4">
      ...
      <div class="text-sm text-gray-400 mt-2">
        Última actualización: {{ sensor?.lastReading?.timestamp ? DateUtils.formatDate(sensor.lastReading.timestamp) : 'Nunca' }}
      </div>
    </div>
  </div>
</template> 